...
class TestRetry(unittest.TestCase):
    ...
    def test_retry3(self):
        ...
        retry.add_call_history_entry(1, {2: ("a", "400", 0.4, 0.5), 3: ("a", False, 0.7, 0.9), 1: ("b", False, 0.1, 0.2)})
    ...
    def test_retry4(self):
        ...
        retry.add_call_history_entry(1, {2: ("a", "400", 0.4, 0.5), 3: ("a", False, 0.7, 0.95), 1: ("b", False, 0.1, 0.2)})
    ...
    def test_retry6(self):
        ...
        retry.add_call_history_entry(1, {4: ("b", False, 0.7, 0.9), 1: ("a", "400", 0.1, 0.2), 3: ("a", "400", 0.4, 0.5)})
    ...
    def test_retry7(self):
        ...
        retry.add_call_history_entry(1, {4: ("b", False, 0.7, 0.9), 1: ("a", "400", 0.1, 0.2), 3: ("a", False, 0.55, 0.6), 2: ("a", True, 0.4, 0.5)})

        retry.detect_retry()
        self.assertEqual(retry.has_retry(), True)
        self.assertEqual(1, len(retry.retry_sequences))
        self.assertListEqual(retry.retry_sequences[0].sequence, [(0.1, 0.2, "400"), (0.4, 0.5, True), (0.55, 0.6, False)])

    def test_retry8(self):
        retry = Retry()
        # Two operation calls on "a": first error second try also error, after that same situation with  "b"
        retry.add_call_history_entry(1, {4: ("b", True, 0.7, 0.9), 1: ("a", "400", 0.1, 0.2), 3: ("a", "400", 0.4, 0.5), 5: ("b", True, 1.1, 1.2)})
    ...
    def test_no_retry4(self):
        ...
        retry.add_call_history_entry(1, {1: ("a", "400", 0.1, 0.2), 3: ("a", False, 0.7, 0.9), 2: ("b", False, 0.4, 0.5)})
    ...
    def test_no_retry5(self):
        ...
        retry.add_call_history_entry(1, {2: ("b", False, 0.4, 0.5), 1: ("a", "400", 0.1, 0.2), 3: ("a", "400", 0.7, 0.9)})

...
def calculate_and_add_demands_with_librede(model: IModel):
    ...
    results: dict[LibredeServiceOperation, float] = librede_output_parser.get_results_of_librede()
    for operation in results.keys():
        add_demand_to_operation(operation, results[operation], model)


def add_demand_to_operation(librede_service_operation: LibredeServiceOperation, demanded_utilization: float, model: IModel):
    ...
    demand: int = int(demanded_utilization * service.capacity)

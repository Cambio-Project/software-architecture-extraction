...
# for a single operation are needed.
class LibReDE_Service:
    ...
    # Adds a new response time to this service. This means another span was found in the trace which represents that this operation ran on it's host.
    ...
    # Parses the response-times in a .csv-format for LibReDE looking like:
...
# Returns a mapping between each operation_name and all its services.
def get_services(trace, hosts: list[LibReDE_Host]) -> dict[str, list[LibReDE_Service]]:
    spans = trace["data"][0]["spans"]
    all_found_services = dict[str, list[LibReDE_Service]]()
    for span in spans:
        service_to_update = get_single_service_to_update(span, hosts, all_found_services)
        new_response_time: tuple[int, int] = (span["startTime"], span["duration"])
...
def get_single_service_to_update(current_span, hosts, all_found_services) -> LibReDE_Service:
    operation_name: str = current_span["operationName"]
    process_id: str = current_span["processID"]
    host: LibReDE_Host = get_host_with_name(process_id, hosts)  # host-object of the current span
    # Registers the operation in the dict, if it hasn't been registered (wasn't part of a span in the trace), yet.
